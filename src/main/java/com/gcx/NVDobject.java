...
public class NVDobject {
    ...
    public static void quickUpdate(String update_file, String output_file) {
        ...
        if (db_exists) {
            ...
            if (new_out != null) System.setOut(new_out);
            // Parsing CVE and CPE data from input file
            List<com.gcx.mitre.cve.CVEobject> cve_objs = com.gcx.mitre.cve.CVEobject.CVEjsonToObjects(update_file);
            // List for removing objects that will be updated now later on
            List<com.gcx.mitre.cve.CVEobject> cves_to_remove = new ArrayList<>();
            for (com.gcx.mitre.cve.CVEobject cve_obj : cve_objs) {
                // Getting specific CVE from the database
                com.gcx.mitre.cve.CVEobject cve_db = session.get(com.gcx.mitre.cve.CVEobject.class, cve_obj.getMeta_data_id());
                if (cve_db != null) {
                    refresh++;
                    // Ensuring optimalization
                    if (refresh % 150 == 0) {
                    ...
                    // Quickly updating CVE and CPE objects in the database
                    com.gcx.mitre.cve.CVEobject.quickUpdateCore(session, cve_db, cve_obj);
                }
            }
            // Committing transaction, closing session
            if (txv.isActive()) txv.commit();
            ...
            cve_objs.removeAll(cves_to_remove);
            // Putting all new CVE objects into database
            com.gcx.mitre.cve.CVEobject.putIntoDatabaseCore(cve_objs, sf);
    ...
    public static void basicDBcore(String cpe_file, String[] cve_files, SessionFactory sf) {
        ...
        com.gcx.mitre.cpe.CPEobject.putIntoDatabase(cpe_file, sf); // file - https://nvd.nist.gov/feeds/json/cpematch/1.0/nvdcpematch-1.0.json.zip

        // Putting CVE objects into database
        com.gcx.mitre.cve.CVEobject.putIntoDatabase(cve_files, sf);
    }
    ...
    public static void flushDB(SessionFactory sf) {
        ...
        session.createSQLQuery("DROP TABLE IF EXISTS mitre.alternate_term CASCADE;" +
                ...
                "DROP TABLE IF EXISTS mitre.cvss3 CASCADE;").executeUpdate();
    ...
    public static Configuration getConfiguration (Boolean update) {
        ...
        if (update && db_exists) {
            ...
            con = new Configuration().configure().addAnnotatedClass(com.gcx.mitre.cve.CVEobject.class).addAnnotatedClass(com.gcx.mitre.cpe.CPEobject.class)
                    .addAnnotatedClass(com.gcx.mitre.cvss.CVSS2object.class).addAnnotatedClass(com.gcx.mitre.cvss.CVSS3object.class).addAnnotatedClass(com.gcx.mitre.cpe.CPEnodeObject.class)
                    .addAnnotatedClass(com.gcx.mitre.cve.ReferenceObject.class).addAnnotatedClass(com.gcx.mitre.cpe.CPEcomplexObj.class).addAnnotatedClass(com.gcx.mitre.cpe.CPEnodeToCPE.class)
